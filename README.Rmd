...
ggbash is a higher-level language for [ggplot2](https://github.com/tidyverse/ggplot2)
and provides a simpler syntax to write ggplot2 objects.

The goal of ggbash is to make ggplot2 more comfortable to write for every ggplot2 user.

ggbash features a strong partial match for experienced ggplot2 users
and helpful error messages for beginners.
It also features handy builtin commands such as `copy` or `png`.
...
instead of bluntly returning an error by Partial Match
(prefix match and fuzzy match).
...
`PpackageVersion('rly')` should be at least 1.4.2.
