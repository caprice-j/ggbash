...
For example, if the input is `p Sepal.W Sepal.L c=Sp`,
...
`gg iris p 2 1 c=5 si=4` is just **16 characters -- more than 80% keystroke reduction**.
...
ggbash interprets only the `|` symbol as the same pipe operator.
There is no functional difference between the two.
You can use one of both you feel more intuitive for each usecase.
...
`png` and `pdf` could receive plot size and file name.
...
generates the same png file `"my-iris-plot.960x480.png"`.
