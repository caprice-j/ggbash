...
In the above example (`p Sepal.W Sepal.L c=Sp si=Petal.W`),
ggbash performs partial matches seven times.

* __geom names__
    + `p` matches `geom_point`
        + Note: the common prefix geom_ is removed beforehand for simplicity.
...
`ggbash` tries to execute its best guess
instead of bluntly returning an error.
Everything is designed to achieve least expected keystrokes.

For example, in the input `p Sepal.W Sepal.L c=Sp s=Petal.W`,
...
Thus, `s` is executed as the `size` aesthetic.
...
While ggplot2 library only interprets `%>%` operator as pipe,
ggbash interprets both `+` and `|` symbols as the same pipe operator.
...
ggplot(mtcars) + geom_point(aes(x=mpg, y=wt)) + geom_smooth(aes(x=mpg, y=wt))
```
...
`png` could receive plot size and file name.
If none specified, the default values are used.

`png` command interprets a single- or double-quoted token as file name
("iris-for" in the following example), and otherwise plot size.
`png` is order-agnostic. Both of the following notations
generates the same png file whose size is 960 pixels in width and 480 pixels in height.
...
If you would like to change the dpi, you could consider `ggplot2::ggsave` function.
...
Therefore, these [document](http://docs.ggplot2.org/current/) and
[book](https://github.com/hadley/ggplot2-book)
are good ways to learn ggplot2 library.
