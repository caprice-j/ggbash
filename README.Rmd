...
* __geom names__
    + `p` matches `geom_point`
        + Note: the common prefix geom_ is removed beforehand.
...
(the first geom, `point`, is selected in this example).

Similarly, `s` matches three aesthetics,
`size`, `shape`, and `stroke`, preferred by this order.
Thus, `s` is interpreted as the `size` aesthetic.
...
`png` can receive three arguments: plot size,
file name, and resolution (pixels per inch).
...
the `pdf` command in ggbash recognizes both inches and pixels as heights.
...
The `png` function in R saves a plot in `Rplot001.png` if no file name is specified.
That function can easily overwrite the previous plot, and users often set file names manually.
...
the output of `p Sepal.W Sepal.L | png` is saved in `iris-150/point_x-Sepal.Width_y-Sepal.Length.480x480.72.png` (72 is the default pixels per inch in R).
