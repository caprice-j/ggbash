...
ggplot(iris)+geom_point(aes(Sepal.Width,Sepal.Length,colour=Species,size=Petal.Width))+theme(text=element_text(size=20,face="bold"))
...
For example, if the input is `ggplot iris + p Sepal.Width Sepal.Length colour=Species`,
...
While it's possible to check and define your own precedence order
...
(`"my-plot"` in the following example), a token with `/` suffix as directory name,
...
It can be categorized into two different subgoals:
...
+ TODO:
    + stat_..., scale_..., coord_..., facet_..., labs, position_...
