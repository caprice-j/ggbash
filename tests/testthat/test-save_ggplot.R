...
test_that('save png', {
    ...
    oldwd <- setwd(tmpdir)
    on.exit(setwd(oldwd))
    ggstr <- 'ggplot2::ggplot(mtcars) + ggplot2::geom_point(ggplot2::aes(cyl,mpg))'
    ...
    for (dev in c('png', 'pdf')) {
        ...
        e(save_ggplot(datadirv[1], ggstr, conf, argv=c(dev, '500x250', '"my-filename"')))
        argv <- c('500x250', '"my-filename"')
        for (indices in list(c(1,2), c(2,1)))
            e(save_ggplot(datadirv[1], ggstr, conf, argv=c(dev, argv[indices])))
