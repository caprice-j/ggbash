library(ggbash)
library(futile.logger)
context("ggbash-lexer-and-perser")

glex  <- rly::lex(Ggplot2Lexer)

ee <- testthat::expect_equal

`%+%` <- function(left, right) paste0(left, right)

test_that("ggplot2 lexer", {

    glex$input("gg iris + point abc def + smooth ghi jkl")
    out <- glex$token()
    ee(out$type,  "GGPLOT")
    ee(out$value, "ggplot2::ggplot(iris")
    ee(out$lineno, 1)
    ee(out$lexpos, 1)
    ee(glex$token()$value, " + geom_point")
    ee(glex$token()$value, "abc")
    ee(glex$token()$value, "def")
    ee(glex$token()$value, " + geom_smooth")
    # Note: improper regex for GGPLOT sometimes
    #       results in ggplot(hi (all characters before g"hi" are replaced)
    ee(glex$token()$value, "ghi")
    ee(glex$token()$value, "jkl")
    ee(glex$token()$value, NULL)

    glex$input(paste0("gg mtcars + point cyl mpg colour=\"blue\"",
                      " size=4 + smooth colour=\"blue\""))
    ee(glex$token()$value, "ggplot2::ggplot(mtcars")
    ee(glex$token()$value, " + geom_point")
    ee(glex$token()$value, "cyl")
    ee(glex$token()$value, "mpg")
    ee(glex$token()$value, "colour=\"blue\"")
    ee(glex$token()$value, "size=4")
    ee(glex$token()$value, " + geom_smooth")
    ee(glex$token()$value, "colour=\"blue\"")
    ee(glex$token()$value, NULL)
})

test_that("ggplot2 parser", {
    prsr <- rly::yacc(Ggplot2Parser)
    ee(prsr$parse("gg iris", glex), "ggplot2::ggplot(iris)")
    ee(prsr$parse("gg iris Sepal.Width", glex),
       "ggplot2::ggplot(iris, ggplot2::aes(Sepal.Width))")
    ee(prsr$parse("gg iris Sepal.Width Sepal.Length", glex),
       "ggplot2::ggplot(iris, ggplot2::aes(Sepal.Width, Sepal.Length))")
    ee(prsr$parse("gg iris Sepal.Width Sepal.Length + point", glex),
       "ggplot2::ggplot(iris, ggplot2::aes(Sepal.Width, Sepal.Length)) + " %+%
       "ggplot2::geom_point()")
    # FIXME how should I deal with non-existing column names?
    ee(prsr$parse("gg iris SepalWidth + point", glex),
       "ggplot2::ggplot(iris, ggplot2::aes()) + ggplot2::geom_point()")

    pre <- "ggplot2::ggplot(iris) + ggplot2::geom_"
    ee(prsr$parse("gg iris + point", glex), pre %+% "point()")
    ee(prsr$parse("gg iris + point Sepal.W Sepal.L", glex),
       pre %+% "point(ggplot2::aes(x=Sepal.Width, y=Sepal.Length))")
    ee(prsr$parse("gg iris + rect Sepal.W Sepal.L Petal.L Petal.W", glex),
       pre %+% "rect(ggplot2::aes(xmin=Sepal.Width, xmax=Sepal.Length, " %+%
           "ymin=Petal.Length, ymax=Petal.Width))")
    #ee(prsr$parse("gg iris + rect Sepal.W Sepal.L", glex),
    #   "rect(ggplot2::aes(ymin=Sepal.Width, ymax=Sepal.Length))")
    # FIXME only last 2 aes are specified

    ee(prsr$parse("gg iris + point Sepal.W Sepal.L + smooth", glex),
       pre %+% "point(ggplot2::aes(x=Sepal.Width, y=Sepal.Length)) + " %+%
           "ggplot2::geom_smooth()")
    ee(prsr$parse("gg iris + point Sepal.W Sepal.L + smooth Sepal.W", glex),
       pre %+% "point(ggplot2::aes(x=Sepal.Width, y=Sepal.Length)) + " %+%
           "ggplot2::geom_smooth(ggplot2::aes(y=Sepal.Width))")
    ee(prsr$parse("gg iris + point Sepal.W Sepal.L + smooth Sepal.W Sepal.L",
                  glex),
       pre %+% "point(ggplot2::aes(x=Sepal.Width, y=Sepal.Length)) + " %+%
           "ggplot2::geom_smooth(ggplot2::aes(x=Sepal.Width, y=Sepal.Length))"
    )

    prsr_pre <- "gg iris + point Sepal.W Sepal.L colour=\"blue\""
    pre <- "ggplot2::ggplot(iris) + ggplot2::geom_point" %+%
        "(ggplot2::aes(x=Sepal.Width, y=Sepal.Length)," %+%
        " colour=\"blue\""
    ee(prsr$parse(prsr_pre, glex), pre %+% ")")
    ee(prsr$parse(prsr_pre %+% " size=4", glex), pre %+% ", size=4)")
    ee(prsr$parse(prsr_pre %+% " size=4 + smooth Sepal.W Sepal.L", glex),
        pre %+%
        ", size=4) + ggplot2::geom_smooth(" %+%
           "ggplot2::aes(x=Sepal.Width, y=Sepal.Length))")
    ee(prsr$parse(prsr_pre %+% " size=4 + smooth colour=\"blue\"", glex),
       pre %+% ", size=4) + ggplot2::geom_smooth(colour=\"blue\")")
    ee(prsr$parse("gg iris + p Sepal.W Sepal.L c=\"blue\" " %+%
        "si=4 + sm c=\"blue\"", glex),
       pre %+% ", size=4) + ggplot2::geom_smooth(colour=\"blue\")")

    ee(prsr$parse("gg iris Sepal.W Sepal.L + geom_point + geom_smooth", glex),
       "ggplot2::ggplot(iris, ggplot2::aes(Sepal.Width, Sepal.Length)) + " %+%
       "ggplot2::geom_point() + ggplot2::geom_smooth()")

    prsr$parse("gg iris  +  point Sepal.W Sepal.L col=Spec siz=Petal.W", glex)
})

test_that("ggplot2 tokenize theme", {
    glex$input("gg mtcars + point cyl mpg + theme text: colour=\"blue\"")
    glex$token()
})

my_pattern <-
  "ggplot2::ggplot\\(iris\\) \\+ ggplot2::theme\\(text = ggplot2::element_text"
test_that("ggplot2 parse theme", {
    prsr <- rly::yacc(Ggplot2Parser)
    glex$input("gg iris + theme text: size=4")
    expect_true(grepl(my_pattern,
                      prsr$parse("gg iris + theme text: size=4", glex)))
    expect_true(
        grepl(my_pattern,
              prsr$parse("gg iris + theme text: size=4 color=\"red\"", glex)))
})
